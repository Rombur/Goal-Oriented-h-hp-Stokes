  ...
  if (parameters.get_quadrature()==gauss_lobatto)
    {
      ...
      for (unsigned int degree=1; degree<=max_degree; ++degree)
        fe_collection.push_back(FESystem<dim>(FE_Q<dim> (QGaussLobatto<1> (/*number of integration points=*/degree+2)), dim,
      ...
      for (unsigned int degree=1; degree<=max_degree; ++degree)
        {
          ...
          quadrature_collection_error.push_back(QGauss<dim> (degree+3));
        }
//It is better to update the Number of quadrature points with polynomial degrees
// not setting the n_quad_points!
      /*
    ...
    {
      for (unsigned int degree=1; degree<=max_degree; ++degree)
        fe_collection.push_back(FESystem<dim>(FE_Q<dim> (/*velocity polynomial degree=*/degree+1), dim,
                                              FE_Q<dim> (/*pressure polynomial degree=*/degree), 1));
          ...
          for (unsigned int i=0; i<dofs_per_cell; ++i)
            {
              if (primal_dual==primal)
                ...
                // For the dual case, the sign of the term phi_p*div_phi_u is
                // different.
                for (unsigned int j=0; j<dofs_per_cell; ++j)
                  local_matrix(i,j) += (scalar_product(grad_phi_u[i], grad_phi_u[j])
                                        - div_phi_u[i]*phi_p[j] - phi_p[i]*div_phi_u[j]) * JxW_values[q];
...
// In Ex-2 since mean-pressure=0, so that it does not matter we subtract mean or not!
// For the uniqueness of pressure we already fixed the first DoFs
...
void StokesProblem<dim>::compute_error()
{
  ...
  std::vector<double> Err_pressure_values;
  std::vector<Tensor<2,dim>> velocity_gradients;
  std::vector<Tensor<2,dim>> Err_velocity_gradients;
  std::vector<Tensor<1,dim>> velocity_values;
  std::vector<Tensor<1,dim>> Err_velocity_values;
  ...
  for (unsigned int cell_index=0; cell!=endc; ++cell,++cell_index)
    {
      double L2_Square_pressure_error = 0.;
      double L2_square_grad_u_error = 0.;
      double L2_square_velocity_error = 0.;
      hp_fe_values.reinit(cell);
      const FEValues<dim> &fe_values = hp_fe_values.get_present_fe_values ();
      const std::vector<double> &JxW_values = fe_values.get_JxW_values ();
      ...
      Err_velocity_values.resize(n_q_points);
      velocity_gradients.resize(n_q_points);
      Err_velocity_gradients.resize(n_q_points);
      pressure_values.resize(n_q_points);
      Err_pressure_values.resize(n_q_points);
      ...
      fe_values[velocities].get_function_gradients(solution, velocity_gradients);
      ...
      for (unsigned int q=0; q<n_q_points; ++q)
        {
          Err_pressure_values[q] = pressure_values[q] - exact_solution_values[q](dim);
          L2_Square_pressure_error += Err_pressure_values[q]*Err_pressure_values[q]* JxW_values[q];

          for (unsigned int i=0; i<dim; ++i)
            {
              Err_velocity_values [q][i] = velocity_values[q][i] - exact_solution_values[q](i);
              Err_velocity_gradients [q][i] = velocity_gradients[q][i] - exact_solution_gradients[q][i];
            }

          L2_square_grad_u_error += Err_velocity_gradients[q].norm_square() * JxW_values[q];
          L2_square_velocity_error += Err_velocity_values[q].norm_square() * JxW_values[q];
        }
      error_per_cell[cell_index] = std::sqrt(L2_Square_pressure_error + L2_square_grad_u_error);
      Vect_Pressure_Err[cell_index] = std::sqrt(L2_Square_pressure_error);
      Vect_grad_Velocity_Err[cell_index] = std::sqrt(L2_square_grad_u_error);
      Vect_Velocity_Err[cell_index] = std::sqrt(L2_square_velocity_error);
    }
...
template <int dim>
void StokesProblem<dim>::compute_goal_error()
{
  ...
  double sum_goal_error = 0. ;

  std::vector<Vector<double>> exact_solution_values;
  std::vector<Vector<double>> dual_rhs_values;
  ...
  for (unsigned int cell_index=0; cell!=endc; ++cell,++cell_index)
    {
      ...
      std::vector<double> Err_Velocity_dual_source(n_q_points, 0.);
      Err_velocity_values.resize(n_q_points);
      ...
      dual_rhs_values.resize(n_q_points, Vector<double>(dim+1));
      dual_source->vector_value_list(quadrature_points, dual_rhs_values);
          ...
          for (unsigned int i=0; i<dim; ++i)
            {
              ...
              Err_velocity_values [q][i] = velocity_values[q][i] - exact_solution_values[q](i);
              // (u-u_h)(x_q) * j(x_q)
              Err_Velocity_dual_source[q]+= Err_velocity_values[q][i]*dual_rhs_values[q](i);

            }

          dual_term +=  Err_Velocity_dual_source[q] *JxW_values[q];
        }
      sum_goal_error += dual_term ;
    }

  std::cout<< std::endl;
  double goal_error = std::fabs(sum_goal_error);
  std::cout<< " Goal_Oriented_Error: "<< goal_error<< std::endl;
}
                                               ...
                                               PRIMALDUAL primal_dual)
{
  ...
  std::vector<double> div_velocity_solu;
  std::vector<double> pressure_solu;
  std::vector<Tensor<2,dim>> grad_velocity_solu;
      ...
      if (dofs_per_cell!=0)
        {
          ...
          div_velocity_solu.resize(n_q_points);
          pressure_solu.resize(n_q_points);
          grad_velocity_solu.resize(n_q_points);

          fe_values[velocities].get_function_divergences(local_solu, div_velocity_solu);
          fe_values[pressure].get_function_values(local_solu, pressure_solu);
          fe_values[velocities].get_function_gradients(local_solu, grad_velocity_solu);
              ...
              for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                  ...
                  double phi_uXrhs_val(0.);
                  double div_phiXpressure_solu(0.);
                  double phi_pXdiv_velocity_solu(0.);
                  double grad_phi_uXgrad_velocity_solu(0.);

                  for (unsigned int d=0; d<dim; ++d)
                    phi_uXrhs_val += phi_u[i][d]*rhs_values[q][d];

                  grad_phi_uXgrad_velocity_solu += scalar_product( grad_phi_u[i] , grad_velocity_solu[q]);
                  div_phiXpressure_solu += div_phi_u[i]*pressure_solu[q];
                  phi_pXdiv_velocity_solu += phi_p[i] * div_velocity_solu[q];

                  local_rhs_patch[i] +=( phi_uXrhs_val - grad_phi_uXgrad_velocity_solu + div_phiXpressure_solu +phi_pXdiv_velocity_solu ) * JxW_val;
...
// This function calculates \grad(W_u) and W_p -- Used further to calculate the \tilde(\eta(K))
template <int dim>
  ...
  BlockVector<double> const &dual_residual_solution)
{
  ...
  //std::vector<Tensor<1,dim> > term_1;
  ...
  for (auto cell : local_dual_dof_handler.active_cell_iterators())
    {
      // to make sure no cell gets zero DoFs!
//      DoFHandler_active_cell_iterator global_cell = patch_to_global_tria_map[cell];
      //    cell->set_active_fe_index(global_cell->active_fe_index());
      hp_fe_values.reinit(cell);
          ...
          for (unsigned int q=0; q<n_q_points; ++q)
            {
              ...
              for (unsigned int i=0; i<dim; ++i)
                {
                  dual_solution_gradients_p[q][i] -= dual_source_values[q][i] + dual_solution_laplacians_v[q][i]
                                                     + dual_residual_laplacians_v[q][i];
                }
              residual_term += pow((cell->diameter())/(cell->get_fe().degree), 2.0) * (scalar_product(dual_solution_gradients_p[q],dual_solution_gradients_p[q])*JxW_values[q]);
      ...
      square_error_estimator +=  jump_term +residual_term ;

    }
  return square_error_estimator;
}

// This function calculated the goal-oriented err est (dual weight*primal estimator)--\pho_k^2 * \eta_k^2
// This is \pho^2(=\tilda(\eta)(K)^2+norm square grad-velocity-local-dual + norm square pressure-local-dual)  * primal-estimator^2
template <int dim>
  ...
  for (unsigned int i=0; cell!=endc; ++cell, ++i)
    {
      ...
      // The first component if the pair is \zeta_k^2=\pho_k^2 * \eta_k^2
  ...
  const &go_error_estimator_square)
{
  ...
  for (auto const &error_estimator : go_error_estimator)
    L2_norm += std::pow(std::get<0>(error_estimator), 2.);

  L2_norm = std::sqrt(L2_norm);
  std::cout<< std::endl;
  std::cout<< "L2-norm-goal-estimator : " << L2_norm << std::endl;
  std::cout<< std::endl;
  double sum(0.);
  for (auto const &error_estimator : go_error_estimator)
    {
      sum += std::pow(std::get<0>(error_estimator), 2);
